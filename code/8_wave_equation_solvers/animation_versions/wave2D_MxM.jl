...
"""
    gradient_2D(U, D1x, D1yT)

Compute the spatial gradients of U[:,:,0,n] (position field).
Returns dudx, dudy.
"""
function gradient_2D(U::OffsetArray{T}, D1x::OffsetArray{T}, D1yT::OffsetArray{T}) where {T}
    return D1x * U[:, :, 0], U[:, :, 0] * D1yT
end
...
function main(T::Type=Float32)
    ...
    # --- Time stepping ---
    println("Starting 2D MxM simulation with $stencil-node stencil and $nodetype nodes…")
    ...
    # --- Output directory ---
    output_dir = "code/8_wave_equation_solvers/animation_versions/output/output2D_$(Nx)_MxM"
    ...
    for n in 0:Nt
        dudx, dudy = gradient_2D(U[:, :, :, n], D1x, D1yT)
        Ek, Ep = zero(T), zero(T)
    ...
    savefig(joinpath(output_dir, "energy_MxM.png"))
    println("Energy plot saved to $(joinpath(output_dir, "energy_MxM.png"))")
    ...
    gif(anim, joinpath(output_dir, "original_MxM.gif"), fps=30)
    println("Animation saved to $(joinpath(output_dir, "original_MxM.gif"))")
    ...
    gif(anim_interp, joinpath(output_dir, "evaluated$(Nfine)_MxM.gif"), fps=30)
    println("Interpolated animation saved to $(joinpath(output_dir, "evaluated$(Nfine)_MxM.gif"))")
end
