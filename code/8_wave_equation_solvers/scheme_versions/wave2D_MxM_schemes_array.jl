# File: 8_wave_equation_solvers/benchmark_versions/wave2D_MxM_benchmark_unified.jl
...
function create_output_directory(Nx, scheme_name, T, use_gpu)
    base_dir = use_gpu ? "code/8_wave_equation_solvers/scheme_versions/output/wave2D_MxM_GPU" :
               "code/8_wave_equation_solvers/scheme_versions/output/wave2D_MxM_CPU"
...
function save_simulation_info(output_dir, scheme_name, T, Nx, Ny, dt, Nt, execution_time, nodetype, use_gpu)
    ...
    open(info_file, "w") do f
        ...
        println(f, "Method: MxM (Matrix-Matrix) on $backend_str")
...
Compute Laplacian of the position field (U[:,:,1]) for 3D arrays.
"""
function laplacian_2D(U::AbstractArray{T,3}, D2x::AbstractArray{T,2}, D2yT::AbstractArray{T,2}) where {T}
    return D2x * U[:, :, 1] .+ U[:, :, 1] * D2yT
end
...
function main(T::Type, Nx::Int, Ny::Int, scheme_name::String, scheme_function, use_gpu::Bool)
    ...
    backend_str = use_gpu ? "GPU" : "CPU"
    println("Starting 2D MxM benchmark with $(stencil)-node stencil and $nodetype nodes on $backend_str…")
