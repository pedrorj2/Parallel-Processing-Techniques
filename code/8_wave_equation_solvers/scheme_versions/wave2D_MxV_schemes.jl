...
# -------------------------------------------------
# Helper functions (idénticas al MxM)
# -------------------------------------------------
"""
...
function create_output_directory(Nx, scheme_name, T)
    base_dir = "code/8_wave_equation_solvers/scheme_versions/output/wave2D_MxV"
...
function save_original_plot_at_time!(U::OffsetArray{T,3}, xnodes, ynodes, time_val::T, A::T, Nx, Ny, nodetype, output_dir, scheme_name) where {T}
    ...
    p = heatmap(x_vals, y_vals, U[:, :, 0]',
                xlabel="x", ylabel="y",
                title=@sprintf("%s MxV: State at t=%.2f s", scheme_name, time_val),
                clim=(-A, A), color=:inferno, size=(600, 600),
                fontsize=12)
    
    filename = @sprintf("%s_MxV_state_t_%.2f.png", scheme_name, time_val)
...
function save_plots_at_time!(U::OffsetArray{T,3}, xnodes, ynodes, time_val::T, A::T, Nx, Ny, nodetype, output_dir, scheme_name) where {T}
    ...
    p = heatmap(xfine, yfine, u_fine',
                ...
                xlabel="x", ylabel="y",
                title=@sprintf("%s MxV: Interpolated state at t=%.2f s", scheme_name, time_val),
                size=(600, 600),
                fontsize=12)
    
    interp_filename = @sprintf("%s_MxV_interpolated_state_t_%.2f.png", scheme_name, time_val)
...
function save_simulation_info(output_dir, scheme_name, T, Nx, Ny, dt, Nt, execution_time, nodetype)
    ...
    open(info_file, "w") do f
        ...
        println(f, "Method: MxV (Matrix-Vector)")
...
Compute Laplacian of the position field (U[:,:,0]) using MxV loops.
ÚNICA DIFERENCIA con MxM: esta función usa loops en lugar de MxM.
"""
function laplacian_2D_MxV(U::OffsetArray{T,3}, D2x::OffsetArray{T,2}, D2y::OffsetArray{T,2}, Nx, Ny) where {T}
    ...
    @inbounds @threads for j in 0:Ny
        for i in 0:Nx
            ...
            d2x = zero(T)
            for ii in 0:Nx
                d2x += D2x[i, ii] * U[ii, j, 0]
            end
            # d²u/dy² at point (i,j)
            d2y = zero(T)
            for jj in 0:Ny
                d2y += D2y[j, jj] * U[i, jj, 0]
            end
            L[i, j] = d2x + d2y
        end
    end
...
"""
    Wave2D_MxV(U, t, params, F)
Right-hand side function for 2D wave equation following F(U,t) pattern.
Returns dU/dt where U contains the state [u, v] fields.
IDÉNTICA al MxM excepto que usa laplacian_2D_MxV.
"""
function Wave2D_MxV(U::OffsetArray{T,3}, t::T, params, F) where {T}
    # Unpack parameters
    D2x, D2y, c, Nx, Ny = params

    apply_dirichlet!(U, Nx, Ny)
    
    # Compute Laplacian usando MxV (ÚNICA DIFERENCIA)
...
# -------------------------------------------------
# Main simulation function (IDÉNTICA al MxM)
# -------------------------------------------------
function main(T::Type, Nx::Int, Ny::Int, scheme_name::String, scheme_function)
    ...
    function rhs(U, t)
        return Wave2D_MxV(U, t, wave_params, F)
    end
