...
"""
    laplacian_2D(U, D2x, D2yT, n)

Compute Laplacian of U[:,:,0,n] (position field).
"""
function laplacian_2D(U::OffsetArray{T}, D2x::OffsetArray{T}, D2yT::OffsetArray{T}, n) where {T}
    # Extraer matrices regulares para evitar problemas con OffsetArrays y BLAS
    u_slice = parent(U[:, :, 0, n])
    d2x_mat = parent(D2x)
    d2yt_mat = parent(D2yT)
    
    # Realizar multiplicación matricial con arrays regulares
    result = d2x_mat * u_slice + u_slice * d2yt_mat
    
    # Retornar como OffsetArray con los mismos índices
    return OffsetArray(result, axes(U[:, :, 0, n]))
end
...
function main(T::Type, Nx::Int, Ny::Int)
    ...
    # Output directory
    output_dir = "code/8_wave_equation_solvers/benchmark_versions/output/output2D_$(Nx)_MxM_benchmark"
    ...
    for n in 0:Nt-1
        ...
        L = laplacian_2D(U, D2x, D2yT, curr)
