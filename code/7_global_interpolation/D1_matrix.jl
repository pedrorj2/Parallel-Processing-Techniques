# File: 7_global_interpolation/D1_matrix.jl
...
"""
    build_D1_matrix(xnodes, stencil_size, T=Float64) -> OffsetArray{T, 2}

Build the D1 matrix for first-order finite difference approximation using Lagrange interpolation.
Each row corresponds to the weights for approximating the first derivative at a node.
...
function build_D1_matrix(xnodes::OffsetVector{<:Real}, stencil_size::Int, ::Type{T}=Float64) where T<:Real
    ...
    D1 = OffsetArray(zeros(T, M, M), 0:M-1, 0:M-1)  # Initialize matrix with type T.
    ...
    for i in 0:M-1
        ...
        for j in stencil_range
            ...
            # Compute L'_j(x_i) using the recursive function (only need the first derivative).
